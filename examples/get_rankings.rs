...
use rust_cfbd::models::cfb::ranking_query::RankingQueryBuilder;
use std::env;

#[tokio::main]
async fn main() {
    dotenv().ok();
    let params = RankingQueryBuilder::new().year(2024).build();

    let api_key = env::var("CFBD_API_KEY").expect("CFBD_API_KEY not set");

    let client = CfbdClient::new(Config::new(api_key));

    let resp = client.get_rankings(&params).await.unwrap();
